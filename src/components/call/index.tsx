...
// LiveKit room instance (will be created on call start)
...
function Call({ interview }: InterviewProps) {
  ...
  }, [isPracticing, practiceTimeLeft, isStarted, livekitRoom]);
  ...
  const handleJoinCall = async () => {
    ...
    try {
      ...
      newRoom.on(RoomEvent.Disconnected, () => {
        ...
        // Save response on disconnect
        if (!hasSavedRef.current && !isPracticing && roomName) {
        ...
        try {
          ...
          if (message.type === "transcript") {
            if (message.role === "assistant") {
              setLastInterviewerResponse(message.text || "");
              setActiveTurn("agent");
            } else if (message.role === "user") {
              setLastUserResponse(message.text || "");
              setActiveTurn("user");
            }
          }
        } catch (error) {
      ...
      // Enable microphone (muted initially)
      await newRoom.localParticipant.setMicrophoneEnabled(true);
      const audioTrack = newRoom.localParticipant.getTrackPublication(Track.Source.Microphone);
      if (audioTrack?.track) {
  ...
  const handleToggleMute = () => {
    ...
    if (livekitRoom) {
      const audioTrack = livekitRoom.localParticipant.getTrackPublication(Track.Source.Microphone);
      if (audioTrack?.track) {
        if (newState) {
          audioTrack.track.mute();
        } else {
